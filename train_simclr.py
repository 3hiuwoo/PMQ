...
parser.add_argument('--data', type=str, default='chapman', choices=['chapman', 'chapman_lead'], help='the dataset to be used')
...
def main():
    ...
    dir = os.path.join(args.log, f'simclr_{model_name}_{args.data}_{args.batch_size}')
    # dir = args.log
    ...
    # creating views
    trans = load_transforms(task='simclr', dataset_name=args.data)
    ...
    model = load_model(args.model, task='simclr', in_channels=in_channels, out_channels=args.dim, depth=args.depth)
    model.to(device)
