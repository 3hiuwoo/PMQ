...
This script is used to train the model under the MoCo paradigm.

Run the script with the following command:
    python train_moco.py {options}
    
See python train_moco.py -h for training options
'''
import os
import argparse
import torch
...
parser = argparse.ArgumentParser(description='pretraining chosen model on chosen dataset under MoCo paradigm')
...
def main():
    ...
    dir = os.path.join(args.log, f'moco_{args.model}_{args.data}_{args.batch_size}')
    # dir = args.log
    ...
    model = load_model(args.model, task='moco', embeddim=args.embedding_dim)
    model.to(device)
    ...
    # creating views
    trans = load_transforms(task='moco', dataset_name=args.data)
    
    train_loader = load_data(root=args.data_root, task='moco', dataset_name=args.data, batch_size=args.batch_size, transform=trans)
    ...
    print(f'=> running moco for {args.epochs} epochs')
...
def train(train_loader, model, optimizer, epoch, metric, writer, device):
    ...
    for signals, _, _ in bar:
        signals = signals.to(device)
        outputs = model(signals)

        loss = moco_loss(outputs)
